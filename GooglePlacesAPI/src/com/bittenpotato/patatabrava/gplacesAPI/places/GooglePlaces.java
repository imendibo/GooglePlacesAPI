package com.bittenpotato.patatabrava.gplacesAPI.places;
...
import com.bittenpotato.patatabrava.gplacesAPI.places.exception.GooglePlacesException;
...
public class GooglePlaces implements GooglePlacesInterface {
    ...
    @Override
    public List<Place> getPlacesByRadar(double lat, double lng, int limit, Param... extraParams) {
        try {
            String uri = buildUrl(METHOD_RADAR_SEARCH, String.format(Locale.ENGLISH, "location=%f,%f&key=%s",
                    lat, lng, apiKey), extraParams);
    ...
    @Override
    public List<Place> getPlacesByRadar(double lat, double lng, Param... extraParams) {
        return getPlacesByRadar(lat, lng, MAXIMUM_RESULTS, extraParams);
    }
    ...
    public Place addPlace(PlaceBuilder builder, boolean returnPlace, Param... extraParams) {
        try {
            ...
            JSONObject response = new JSONObject(this.requestHandler.post(post));
	...
	public List<Place> getCustomPlaces(double latitude, double longitude, String type, String rank,
			int maximumResults) {
		try {
            String uri = buildUrl(METHOD_NEARBY_SEARCH, String.format(Locale.ENGLISH, 
            		"location=%f,%f&type=%s&rankby=%s&key=%s",
                    latitude, longitude, type, rank, apiKey));
            return getPlaces(uri, METHOD_NEARBY_SEARCH, maximumResults);
        } catch (Exception e) {
            throw new GooglePlacesException(e);
        }
	}
